...
generic<typename T> where T:IComparable array<T>^ RemoveElement(T element, array<T>^ data){
...
generic<typename T> void ListElements(array<T>^ data){
	for each(T item in data){
...
int main(array<System::String ^> ^args){
	...
	array<String^>^ strings = {
		L"Many", L"hands", L"make", L"light", L"work"
	}; 
	Console::WriteLine(L"\nArray contains:");
